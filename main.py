...
def parse_args():
    ...
    # dataset
    parser.add_argument('--dataset', default='sbap_core_ec50_scaffold', type=str, help='name of data set')
    parser.add_argument('--root', default='/mnt/hdd1/caoye/OODv2/processed',
                        type=str, help='root of input data')
    parser.add_argument('--save_graph', default='/mnt/hdd1/caoye/OODv2/processed/graph', type=str,
                        help='path of graphs')
    # Training parameters
    parser.add_argument('--epochs', default=300, type=int, metavar='N', help='number of total epochs to run')
    parser.add_argument('-b', '--batch_size', default=10, type=int, metavar='N',
    ...
    parser.add_argument("--graph_pool", type=str, default='mean', choices=['sum', 'max', 'mean', 'wsum'],
    ...
    parser.add_argument("--lam1", type=float, default=0., help='the weight of causal loss')
...
def validate(args, data_generator, model):
    ...
    y_pred_neg = []
    with torch.no_grad():
        for i, (graph_pos, protein_pos, prot_mask_pos, label_pos, graph_neg, protein_neg, prot_mask_neg, label_neg) in enumerate(data_generator):
            graph_pos, protein_pos, prot_mask_pos, label_pos, graph_neg, protein_neg, prot_mask_neg, label_neg = graph_pos.to(
                args.device), protein_pos.to(args.device), prot_mask_pos.to(args.device), torch.FloatTensor(
                np.array(label_pos)), graph_neg.to(args.device), protein_neg.to(args.device), prot_mask_neg.to(
                args.device), torch.FloatTensor(np.array(label_neg))

            pred_pos = model(graph_pos, protein_pos, prot_mask_pos)
            pred_neg = model(graph_neg, protein_neg, prot_mask_neg)

            y_pred_pos.append(pred_pos)
            y_pred_neg.append(pred_neg)

    y_pred_pos = torch.cat(y_pred_pos, dim=0)
    y_pred_neg = torch.cat(y_pred_neg, dim=0)
    return y_pred_neg.flatten(), y_pred_pos.flatten()
...
def training(args, model, train_dataloader, opt, loss_margin):
    ...
    total_loss = []
    for i, (
    graph_pos, protein_pos, prot_mask_pos, label_pos, graph_neg, protein_neg, prot_mask_neg, label_neg) in enumerate(
            train_dataloader):
        graph_pos, protein_pos, prot_mask_pos, label_pos, graph_neg, protein_neg, prot_mask_neg, label_neg = graph_pos.to(
            args.device), protein_pos.to(args.device), prot_mask_pos.to(args.device), torch.FloatTensor(
            np.array(label_pos)), graph_neg.to(args.device), protein_neg.to(args.device), prot_mask_neg.to(
            args.device), torch.FloatTensor(np.array(label_neg))
        target = torch.ones(len(label_pos)).to(args.device)

        if args.causal_inference:
            pred_pos, c_pred_pos, s_pred_pos, interv_pred_pos = model(graph_pos, protein_pos, prot_mask_pos,
                                                                      training=True)
            pred_pos, c_pred_pos, s_pred_pos, interv_pred_pos = pred_pos.view(-1), c_pred_pos.view(-1), s_pred_pos, \
                                                                interv_pred_pos.view(-1)

            pred_neg, c_pred_neg, s_pred_neg, interv_pred_neg = model(graph_neg, protein_neg, prot_mask_neg,
                                                                      training=True)
            pred_neg, c_pred_neg, s_pred_neg, interv_pred_neg = pred_neg.view(-1), c_pred_neg.view(-1), s_pred_neg, \
                                                                interv_pred_neg.view(-1)
            s_pred = torch.cat([s_pred_neg, s_pred_pos])
            c_pred = torch.cat([c_pred_neg, c_pred_pos])
            interv_pred = torch.cat([interv_pred_neg, interv_pred_pos])
            pred = torch.cat([pred_neg, pred_pos])
            label = torch.cat([label_neg, label_pos]).to(args.device)
            ...
            if args.shortcut_loss == 'KL':
                s_pred = F.log_softmax(s_pred, dim=-1)
                s_loss = F.kl_div(s_pred, target_s, reduction='batchmean')
            if args.shortcut_loss == 'MSE':
                s_loss = F.mse_loss(torch.softmax(s_pred, dim=-1), target_s, reduction='mean')

            loss_global = loss_margin(pred_pos, pred_neg, target)
            loss_inv = loss_fct(torch.sigmoid(interv_pred), label)
            loss_dist = F.kl_div(torch.log(torch.stack((torch.sigmoid(pred), 1 - torch.sigmoid(pred)), dim=1)),
                                 torch.stack((torch.sigmoid(c_pred), 1 - torch.sigmoid(c_pred)), dim=1))
            ...
            pred_pos = model(graph_pos, protein_pos, prot_mask_pos, training=True)
            pred_neg = model(graph_neg, protein_neg, prot_mask_neg, training=True)
            loss = loss_margin(pred_pos.view(-1), pred_neg.view(-1), target)
...
def main(args):
    ...
    for epo in tqdm(range(args.epochs)):
        ...
        # validation
        val_pred_neg, val_pred_pos = validate(args, validation_generator, model)
        target = torch.ones(len(val_pred_pos)).to(args.device)
        val_loss = loss_margin(val_pred_pos, val_pred_neg, target)
        val_pred = torch.cat([val_pred_neg, val_pred_pos])
        val_true = torch.cat([torch.zeros(len(val_pred_pos)), torch.ones(len(val_pred_pos))])
    ...
    print('[INFO] Testing ...')
    test_pred_neg, test_pred_pos = validate(args, testing_generator, model_max)
    test_pred = torch.cat([test_pred_neg, test_pred_pos])
    test_true = torch.cat([torch.zeros(len(test_pred_pos)), torch.ones(len(test_pred_pos))])
